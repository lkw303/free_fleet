...
#ifndef FREE_FLEET__INCLUDE__FREE_FLEET__CLIENTMQTT_HPP
#define FREE_FLEET__INCLUDE__FREE_FLEET__CLIENTMQTT_HPP
...
public:
  ...
  using SharedPtr = std::shared_ptr<ClientMqtt>;

  /// Factory function that creates an instance of the Free Fleet DDS ClientMqtt.
  ///
  /// \param[in] config
  ///   Configuration that sets up the client to communicate with the server.
  /// \return
  ///   Shared pointer to a free fleet client.
  static SharedPtr make(const ClientMqttConfig& config);
...
private:
  ...
  class ClientMqttImpl;

  std::unique_ptr<ClientMqttImpl> impl;

  ClientMqtt(const ClientMqttConfig& config);
