...
#ifndef FREE_FLEET__INCLUDE__FREE_FLEET__SERVER_MQTT_HPP
#define FREE_FLEET__INCLUDE__FREE_FLEET__SERVER_MQTT_HPP
...
public:
  ...
  using SharedPtr = std::shared_ptr<ServerMqtt>;
  ...
  static SharedPtr make(const ServerMqttConfig& config);
...
private:
  ...
  class ServerMqttImpl;

  std::unique_ptr<ServerMqttImpl> impl;

  ServerMqtt(const ServerMqttConfig& config);
