...
ServerNodeMqtt::SharedPtr ServerNodeMqtt::make(
    const ServerNodeMqttConfig& _config, const rclcpp::NodeOptions& _node_options)
{
  // Starting the free fleet server node
  SharedPtr server_node(new ServerNodeMqtt(_config, _node_options));
  ...
  ServerMqtt::SharedPtr server = ServerMqtt::make(server_config);
  if (!server)
    return nullptr;
...
void ServerNodeMqtt::setup_config()
{
  ...
  // Settings for Server
  get_parameter("mqtt_server_address",
      server_node_config.mqtt_server_address);
  get_parameter("mqtt_server_address",
      server_node_config.mqtt_server_address);
  get_parameter("mqtt_robot_state_topic",
      server_node_config.mqtt_robot_state_topic);
  get_parameter("mqtt_mode_request_topic",
      server_node_config.mqtt_mode_request_topic);
  get_parameter("mqtt_path_request_topic",
      server_node_config.mqtt_path_request_topic);
  get_parameter(
      "mqtt_destination_request_topic",
      server_node_config.mqtt_destination_request_topic);
  get_parameter("QOS", server_node_config.QOS);
  get_parameter("LWT_PAYLOAD", server_node_config.LWT_PAYLOAD);

  // Settings for Server Node
...
void ServerNodeMqtt::start(Fields _fields)
{
  ...
  update_state_timer = create_wall_timer(
      100ms, std::bind(&ServerNodeMqtt::update_state_callback, this),
  ...
  fleet_state_pub_timer = create_wall_timer(
      ...
      std::bind(&ServerNodeMqtt::publish_fleet_state, this),
...
bool ServerNodeMqtt::is_request_valid(
...
void ServerNodeMqtt::transform_fleet_to_rmf(
...
void ServerNodeMqtt::transform_rmf_to_fleet(
...
void ServerNodeMqtt::handle_mode_request(
...
void ServerNodeMqtt::handle_path_request(
...
void ServerNodeMqtt::handle_destination_request(
...
void ServerNodeMqtt::update_state_callback()
{
...
void ServerNodeMqtt::publish_fleet_state()
{
