...
#ifndef FREE_FLEET_SERVER_ROS2__SRC__SERVERNODEMQTT_HPP
#define FREE_FLEET_SERVER_ROS2__SRC__SERVERNODEMQTT_HPP
...
class ServerNodeMqtt : public rclcpp::Node
{
public:

  using SharedPtr = std::shared_ptr<ServerNodeMqtt>;
  using ReadLock = std::unique_lock<std::mutex>;
  using WriteLock = std::unique_lock<std::mutex>;

  static SharedPtr make(
      const ServerNodeMqttConfig& config,
...
private:
  ...
  ServerNodeMqttConfig server_node_config;
  ...
  ServerNodeMqtt(
      const ServerNodeMqttConfig& config, const rclcpp::NodeOptions& options);
