...
class ClientNodeMqtt
{
public:

  using SharedPtr = std::shared_ptr<ClientNodeMqtt>;
  ...
  static SharedPtr make(const ClientNodeMqttConfig& config);
...
private:
  ...
  ClientNodeMqttConfig client_node_config;

  Fields fields;

  ClientNodeMqtt(const ClientNodeMqttConfig& config);
