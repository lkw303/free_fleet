...
ClientNodeMqtt::SharedPtr ClientNodeMqtt::make(const ClientNodeMqttConfig& _config)
{
  SharedPtr client_node = SharedPtr(new ClientNodeMqtt(_config));
  client_node->node.reset(new ros::NodeHandle(_config.robot_name + "_node"));

  /// Starting the free fleet client
  ClientMqttConfig client_config = _config.get_client_config();
  ClientMqtt::SharedPtr client = ClientMqtt::make(client_config);
  if (!client)
    return nullptr;
...
ClientNodeMqtt::ClientNodeMqtt(const ClientNodeMqttConfig& _config) :
  tf2_listener(tf2_buffer),
  client_node_config(_config)
{}

ClientNodeMqtt::~ClientNodeMqtt()
{
...
void ClientNodeMqtt::start(Fields _fields)
{
  ...
  battery_percent_sub = node->subscribe(
      ...
      &ClientNodeMqtt::battery_state_callback_fn, this);
  ...
  update_thread = std::thread(std::bind(&ClientNodeMqtt::update_thread_fn, this));

  ROS_INFO("Client: starting publish thread.");
  publish_thread =
      std::thread(std::bind(&ClientNodeMqtt::publish_thread_fn, this));
}

void ClientNodeMqtt::print_config()
{
  client_node_config.print_config();
}

void ClientNodeMqtt::battery_state_callback_fn(
...
bool ClientNodeMqtt::get_robot_transform()
{
  try {
...
messages::RobotMode ClientNodeMqtt::get_robot_mode()
{
...
void ClientNodeMqtt::publish_robot_state()
{
...
bool ClientNodeMqtt::is_valid_request(
...
move_base_msgs::MoveBaseGoal ClientNodeMqtt::location_to_move_base_goal(
...
bool ClientNodeMqtt::read_mode_request()
{
...
bool ClientNodeMqtt::read_path_request()
{
...
bool ClientNodeMqtt::read_destination_request()
{
...
void ClientNodeMqtt::read_requests()
{
...
void ClientNodeMqtt::handle_requests()
{
...
void ClientNodeMqtt::update_thread_fn()
{
  while (node->ok())
  {
    update_rate->sleep();
...
void ClientNodeMqtt::publish_thread_fn()
{
  while (node->ok())
  {
    publish_rate->sleep();
